...
install.packages("reticulate") #python moduleㅎ 가져올수있는거 
...
dataset <- import("sklearn.datasets") #이거 까는방법 
...
library(caret) #data 쪼개는거 (python list 해결 위해서)
  ...
  for (epoch in 1:n_epochs){
    ...
    for (batch_index in 1:n_batches){
      sess$run(training_op, feed_dict=dict(X = scaled_housing_data_plus_bias[folds[[batch_index]],],
                                           y = matrix(housing$target[folds[[batch_index]]]))) #이런부분이 파이썬이랑 조금 달라진 부분 
    }
  }
  best_theta = theta$eval()
})
...
logdir
#tools - > shell
#terminal 에  $tensorboard --logdir tf_logs/run-2019.~~~하면 tensorboard 똑같이 쓸 수 있음 

n_epochs = 1000
learning_rate = 0.01

X = tf$placeholder(tf$float32, shape(NULL, n + 1), name="X") #n?
y = tf$placeholder(tf$float32, shape(NULL, 1), name="y")
...
o = tf$add(b,y,name='output') #위에서 b, y
sess = tf$Session()
